...
class BM25RetrieverTest(TestCase):
    ...
    def test_top_k_is_respected(self):
        retriever = BM25Retriever()
        results = retriever.search("قرارداد", top_k=1)
        self.assertLessEqual(len(results), 1)

    def test_unrelated_query_returns_no_results(self):
        retriever = BM25Retriever()
        results = retriever.search("ورشکستگی", top_k=5)
        self.assertEqual(results, [])

    def test_document_title_is_indexed(self):
        retriever = BM25Retriever()
        results = retriever.search("کار", top_k=3)
        self.assertGreater(len(results), 0)
        self.assertEqual(results[0]["provision"], self.provision_termination)

    def test_search_can_find_provision_by_title(self):
        retriever = BM25Retriever()
        results = retriever.search("بیع", top_k=3)
        self.assertGreater(len(results), 0)
        self.assertEqual(results[0]["provision"], self.provision_sale)
