...
class DenseIndexer:
    ...
    def create_collection(self):
        ...
        schema = CollectionSchema(
            fields=fields,
            description=("Dense embeddings of legal provision groups"),
        )
    ...
    def _get_provision_type_label(self, provision):
        return self.PROVISION_TYPE_LABELS.get(
            provision.provision_type,
            "مقرره",
        )

    def _get_structural_type_label(self, structural):
        return self.STRUCTURAL_TYPE_LABELS.get(
    ...
    def _build_groups(self):
        ...
        # Make sure the root provision is always first.
        for root_id, group in groups.items():
            root = next(
                (provision for provision in group if provision.id == root_id),
                None,
            )
    ...
    def _build_structural_text(
        self,
        provision,
        parent_by_element_id,
        structural_by_element_id,
    ):
        parts = []
    ...
    def _build_root_text(
        ...
        structural_text = self._build_structural_text(
            root_provision,
            parent_by_element_id,
            structural_by_element_id,
        )
    ...
    def index(self, provisions):
        ...
        structural_elements = list(
            StructuralElement.objects.only(
                "element_id",
                "structural_type",
                "number",
                "title",
        ...
        for start in range(
            ...
            total_indexed += self._index_batch(
                batch,
                parent_by_element_id,
                structural_by_element_id,
            )
